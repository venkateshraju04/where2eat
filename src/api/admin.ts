...
export const getPendingSuggestions = createServerFn({ method: "POST" }).handler(
  async ({ data }: { data: any }) => {
    verifyAdmin(data);
...
export const rejectSuggestion = createServerFn({ method: "POST" }).handler(
  async ({ data }: { data: any }) => {
    verifyAdmin(data);
...
export const approveSuggestion = createServerFn({ method: "POST" }).handler(
  async ({ data }: { data: any }) => {
    verifyAdmin(data);
    ...
    const { error: insertErr } = await supabase.from("restaurants").insert([
      {
        ...
        image_url:
          data.imageUrl ||
          "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=1500&auto=format&fit=crop", // placeholder restaurant image
    ...
    // 3. Mark as approved
    await supabase.from("suggestions").update({ status: "approved" }).eq("id", data.id);
