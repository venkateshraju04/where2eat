    ...
    return (
      <div className="min-h-screen bg-gradient-hero flex items-center justify-center p-5">
        ...
        >
          ...
          <button type="submit" className="w-full rounded-xl bg-gradient-primary text-primary-foreground py-3 font-semibold shadow-glow hover:scale-[1.02] transition-smooth">
            Enter
          </button>
        </motion.form>
      </div>
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="max-w-5xl mx-auto px-5 pt-8">
        <div className="flex justify-between items-end mb-8">
          ...
          <button onClick={fetchSuggestions} className="p-2.5 rounded-full bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-smooth">
        ...
        {loading && !suggestions.length ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {suggestions.map(s => (
              <motion.div key={s.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass rounded-2xl p-5 shadow-card flex flex-col">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-xl font-bold">{s.name}</h3>
                    <p className="text-primary text-sm font-medium">{s.cuisine} • {s.area}</p>
                  </div>
                  <div className="bg-secondary px-2 py-1 rounded-md text-xs font-semibold">₹{s.price}</div>
                </div>
                <p className="text-sm text-muted-foreground flex-1 mb-4">{s.description}</p>
                <div className="text-xs text-muted-foreground mb-4">
                  <strong>Suggested by:</strong> {s.submitter_name || "Anonymous"}<br/>
                  <strong>Moods:</strong> {s.suggested_moods?.join(", ") || "None"}
                </div>
                
                <div className="grid grid-cols-2 gap-2 mt-auto">
                  <button onClick={() => handleReject(s.id)} disabled={actionLoading} className="flex justify-center items-center gap-1.5 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-red-500/10 hover:text-red-500 transition-smooth">
                    <X className="w-4 h-4" /> Reject
                  </button>
                  <button onClick={() => setApproveModal(s)} disabled={actionLoading} className="flex justify-center items-center gap-1.5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:scale-[1.02] shadow-glow transition-smooth">
      ...
      <AnimatePresence>
        {approveModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            ...
            >
              <button onClick={() => setApproveModal(null)} className="absolute right-4 top-4 p-2 rounded-full hover:bg-secondary transition-smooth">
                <X className="w-5 h-5 text-muted-foreground" />
              </button>
              
              <h2 className="text-2xl font-bold mb-1">Finalize Data</h2>
              <p className="text-sm text-muted-foreground mb-6">You are approving <strong>{approveModal.name}</strong>.</p>
              ...
              <form onSubmit={handleApproveSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" step="any" required value={lat} onChange={e => setLat(e.target.value)} placeholder="e.g. 12.9716" className="w-full rounded-xl bg-secondary/50 border border-border px-3 py-2 text-sm outline-none focus:border-primary transition-smooth" />
                  </div>
                  <div>
                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1 block">Longitude</label>
                    <input type="number" step="any" required value={lng} onChange={e => setLng(e.target.value)} placeholder="e.g. 77.5946" className="w-full rounded-xl bg-secondary/50 border border-border px-3 py-2 text-sm outline-none focus:border-primary transition-smooth" />
                  </div>
                </div>
                <p className="text-xs text-muted-foreground -mt-2">Grab these from Google Maps URL.</p>
                ...
                <div className="pt-2">
                  <button type="submit" disabled={actionLoading} className="w-full rounded-xl bg-gradient-primary text-primary-foreground py-3 font-semibold shadow-glow hover:scale-[1.02] transition-smooth disabled:opacity-50">
